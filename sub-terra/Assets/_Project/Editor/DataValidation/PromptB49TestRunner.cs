...
{
    /// <summary>prompt-B 49 시설 1칸 비주얼·말풍선 테스트만 실행한다.</summary>
    public static class PromptB49TestRunner
    {
        private static TestRunnerApi api;
        private static ResultWriter receiver;

        [MenuItem("SubTerra/Tests/Run Prompt-B 49 Facility Visual Tests")]
        ...
        public static void RunEditMode()
        {
            ...
            var path = Path.GetFullPath(Path.Combine(
                Application.dataPath, "..", "Temp", "prompt-b49-editmode-results.txt"));
            if (File.Exists(path))
            {
                File.Delete(path);
            }
            ...
            api.Execute(new ExecutionSettings(new Filter
            {
                ...
                groupNames = new[]
                {
                    "SubTerra.App.Tests.UI.PromptB49FacilityVisualTests"
                }
            }));
            Debug.Log("[SubTerra] Prompt-B 49 EditMode tests requested → " + path);
        }
        ...
        private sealed class ResultWriter : ICallbacks
        {
            ...
            public void RunFinished(ITestResultAdaptor result)
            {
                ...
                Debug.Log(
                    "[SubTerra] Prompt-B 49 tests finished pass="
                    + result.PassCount
                    + " fail="
                    + result.FailCount);
                ReleaseRunner(this);
