...
{
    /// <summary>prompt-B 94/94-1/94-2 미니맵 레이아웃·토글 테스트를 실행한다.</summary>
    public static class PromptB94MinimapTestRunner
    {
        ...
        private static void WatchRequest()
        {
            ...
            {
                const string flag = "Temp/subterra-run-prompt-b94.flag";
        ...
        [MenuItem("SubTerra/Tests/Run Prompt-B 94 Minimap Tests")]
        ...
        public static void RunEditMode()
        {
            ...
            var path = Path.GetFullPath(Path.Combine(
                Application.dataPath, "..", "Temp", "prompt-b94-editmode-results.txt"));
            if (File.Exists(path))
            {
                File.Delete(path);
            }
            ...
            receiver = new ResultWriter(path, "Prompt-B 94 EditMode");
            api.RegisterCallbacks(receiver);
            api.Execute(new ExecutionSettings(new Filter
            {
                testMode = TestMode.EditMode,
                assemblyNames = new[] { "SubTerra.App.Tests.EditMode" },
                groupNames = new[] { "SubTerra.App.Tests.UI.PromptB94MinimapTests" }
            }));
            Debug.Log("[SubTerra] Prompt-B 94 EditMode tests requested → " + path);
        }
        ...
        private sealed class ResultWriter : ICallbacks
        {
            ...
            public void RunFinished(ITestResultAdaptor result)
            {
                ...
                Debug.Log($"[SubTerra] Prompt-B 94 finished Pass={passed} Fail={failed} Skip={skipped}");
