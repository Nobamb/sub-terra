...
55-1(구현 예정). 지금 지반 붕괴에 의해 떨어지는 오브젝트에 캐릭터가 맞닿아도 데미지를 받고 있지 않는데 지반 붕괴로 떨어지는 오브젝트와 캐릭터가 충돌했을 때 데미지를 받도록 해주고 데미지를 받을 때마다 화면이 흔들리도록 해줘 물론 감소되는 HP가 클 수록 그 흔들리는 수준이 크도록 해주고 이 흔들림은 설정창에서 화면 진동 억제를 켤 때 없어질 수 있도록 해주면 돼 그리고 좌측의 HUD 표시 포맷의 HP가 다른 표시 텍스트들보다도 왼쪽으로 빠져나왔는데 높이 및 상하 간격은 다른 표시 텍스트들과 왼쪽으로 같은 위치를 유지하도록만 UI 수정해줘 rule.md의 내용을 필수 참고해서 작업을 진행해
